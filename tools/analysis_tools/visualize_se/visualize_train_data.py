...
def viz_test_data_item(data, save_path=None):
    ...
    # past  data['instance_idx_2_labels'][0].data
    i = 0
    for instance in data['instance_idx_2_labels'][0].data.values():
        past_traj=instance['past_traj']
        past_traj_valid=instance['past_traj_is_valid']
        future_traj=instance['future_traj']
        future_traj_valid=instance['future_traj_is_valid']

        past_traj = past_traj[past_traj_valid.astype(np.bool)]
        future_traj = future_traj[future_traj_valid.astype(np.bool)]
        color = tuple(np.random.rand(3,))
        # past
        ax.plot(past_traj[:, 0], past_traj[:, 1], marker='.', alpha=0.5, color=color, zorder=15)
        # orig
...
if __name__ == "__main__":
    ...
    cfg = Config.fromfile("projects/configs/StreamPETR_VIP3D/stream_petr_vov_flash_800_bs2_seq_24e_petrbase.py")
    ...
    if TEST:
        ...
        save_path = "test/viz_test"
    else:
        dataset = build_dataset(cfg.data.train)
        save_path = "test/viz_train"
