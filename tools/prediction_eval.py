...
class GTAgent:
    def __init__(self,
                 ...
                 future_traj_is_valid: np.ndarray = np.zeros(cfg.future_frame_num, dtype=np.int64)
                 ):
...
def get_gt_agents(prediction_infos, index):
    ...
    for i in range(index, index + 1 + cfg.future_frame_num):
        ...
        instance_inds = np.array(info['instance_inds'], dtype=np.int64)
        gt_bboxes_3d = np.array(info['gt_bboxes_3d'])
        gt_labels_3d = np.array(info['gt_labels_3d'], dtype=np.int64)
    ...
    def evaluate(self):
        ...
        for index in tqdm(range(len(self.prediction_infos))):
            ...
            if len(gt_agents) > 0:
                ...
                matched_of_gt_box = np.ones(len(gt_agents), dtype=np.int64) * -1
...
def main():
    ...
    parser.add_argument('--result_path', help='path of prediction results in JSON format', 
                        default="work_dirs/test/stream_petr_vov_flash_800_bs16_wk4_seq_24e_mini/Thu_Nov_30_03_18_32_2023/pts_bbox/results_nusc.json")
    parser.add_argument('--prediction_infos_path',
                        default='/mnt/data/dataset/nuScenes/nuscenes_prediction_infos_val_mini.json',
